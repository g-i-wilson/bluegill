...
public class QuadratureDemodulator implements ComplexProducer {
	...
	public static void main (String[] args) {
		...
		ComplexProducer se = new SignalEnvelope( 16 );
		...
		for (int a=0; a<5; a++) {
			...
			for (int b=0; b<40; b++) {
				...
				Complex p = qd.sample( filteredSample );
				System.out.println(
					modSample+","+
					filteredSample+","+
					p.magnitude()+","+
					p.phase()+","+
					se.sample(filteredSample).real()+","+
					se.sample(filteredSample).imag()+","+
					se.sample(filteredSample).magnitude()
				);
			}
		}
